...
class PwaTests(unittest.TestCase):
    ...
    def test_service_worker_route_returns_expected_script(self) -> None:
        ...
        self.assertIn('const CACHE_NAME = "noesisfood-shell-v3";', content)
        self.assertIn("key !== CACHE_NAME", content)
        self.assertIn("caches.delete(key)", content)
        ...
        self.assertNotIn('const CACHE_NAME = "noesisfood-shell-v2";', content)
    ...
    def test_frontend_links_manifest_and_registers_service_worker(self) -> None:
        ...
        self.assertIn('navigator.serviceWorker.register("/service-worker.js", { scope: "/" }).catch(() => {});', content)
        self.assertIn('window.__NF_BUILD__ = "2026-08-02-licensed-shell-v3";', content)
